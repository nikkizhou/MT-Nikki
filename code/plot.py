...
def generate_and_save_plot(metric_name, metric_data):
    ...
    bar1 = ax.bar(index, metric_data['with_cv'], bar_width, label='With CV', color='#f08080')  
    bar2 = ax.bar(index + bar_width, metric_data['without_cv'], bar_width, label='Without CV', color='#4169e1') 
    ...
    plt.tight_layout()
    file_name = f'plot/{metric_name.lower().replace(" ", "_")}_chart.png'
